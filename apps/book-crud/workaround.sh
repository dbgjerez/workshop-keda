...
oc create secret generic -n dev-backend kafka-connection \
 ...
 --from-literal=user.password=$(oc get secret -n kafka kafka-producer-stock-topic -o json | jq -r '.data["user.password"]' | base64 -d)